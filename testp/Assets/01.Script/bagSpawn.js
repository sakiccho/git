...
public var bSpawn : Transform;
public var scorpion : GameObject;
...
function Spawn () {
    var obj : GameObject = GameObject.Instantiate(scorpion);  //GameObject型変数objを宣言
    obj.transform.position = bSpawn.position; 
	...
	if(!isQuitting){
		if(totalEnemy < maxEnemy) {
		    ...
		    //ランダム生成する際の範囲が-25~25なのは地面の大きさが50だから
		        ...
		        GameObject.Instantiate(scorpion, pos, Quaternion.identity);
		        //第一引数は生成するオブジェクト
		        //第二引数は生成する座標
		        //第三引数は生成したオブジェクトの向き
		        enemyCount += 1;
